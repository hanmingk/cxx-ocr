...
std::vector<int>
OrderPointsClockwise(std::vector<int> pts)
{
    ...
    std::vector<int> rect = {leftmost[0][0], leftmost[0][1], rightmost[0][0], rightmost[0][1], 
                             rightmost[1][0], rightmost[1][1],leftmost[1][0], leftmost[1][1]};
    return rect;
}
                ...
                const int det_use_polygon_score)
{
    ...
    for (int i = 0; i < num_contours; i++)
    {
        ...
        for (int num_pt = 0; num_pt < 4; num_pt++)
        {
            ...
            intcliparray[num_pt * 2 + 1] = static_cast<int>(clamp(
                ...
                float ratio_w, cv::Mat srcimg)
{
    ...
    for (size_t n = 0; n < boxes.size(); n++)
    {
        ...
        for (size_t m = 0; m < 4; m ++)
        {
            ...
            boxes[n][m * 2] = 
                static_cast<int>(std::min(std::max(boxes[n][m * 2], 0), oriimg_w - 1));
            boxes[n][m * 2 + 1] =
                static_cast<int>(std::min(std::max(boxes[n][m * 2 + 1], 0), oriimg_h - 1));
        }
    }
    ...
    for (int n = 0; n < boxes.size(); n++)
    {
        ...
        rect_width =
            static_cast<int>(sqrt(pow(boxes[n][0] - boxes[n][2], 2) +
                                  pow(boxes[n][1] - boxes[n][3], 2)));
        rect_height =
            static_cast<int>(sqrt(pow(boxes[n][0] - boxes[n][6], 2) +
