...
PaddleStructure::PaddleStructure(const Flags &flags) : PPOCR(flags) {
  std::string layout_model_dir(flags.layout_model_dir);
  std::string layout_dict_path(flags.layout_dict_path);
  std::string precision(flags.precision);
  std::string table_model_dir(flags.table_model_dir);
  std::string table_char_dict_path(flags.table_char_dict_path);

  if (flags.layout) {
    this->layout_model_.reset(new StructureLayoutRecognizer(
        layout_model_dir, flags.use_gpu, flags.gpu_id, flags.gpu_mem,
        flags.cpu_threads, flags.enable_mkldnn, layout_dict_path,
        flags.use_tensorrt, precision, flags.layout_score_threshold,
        flags.layout_nms_threshold));
  }
  if (flags.table) {
    this->table_model_.reset(new StructureTableRecognizer(
        table_model_dir, flags.use_gpu, flags.gpu_id, flags.gpu_mem,
        flags.cpu_threads, flags.enable_mkldnn, table_char_dict_path,
        flags.use_tensorrt, precision, flags.table_batch_num,
        flags.table_max_len, flags.merge_no_span_structure));
  }
}
